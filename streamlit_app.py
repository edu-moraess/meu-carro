...
def register_user(email:str,password:str)->tuple[bool,str]:
    ...
    with SessionLocal() as db:
        ...
        code=secrets.token_hex(5).upper();user=User(email=email,password_hash=hash_password(password),trial_started_at=now,trial_ends_at=now+timedelta(days=30),referral_code=code);db.add(user)
...
def ai_request(prompt:str,image_bytes:Optional[bytes]=None,mime_type:str="image/jpeg")->Optional[str]:
    ...
    try:
        r=requests.post(f"https://generativelanguage.googleapis.com/v1beta/models/{GEMINI_MODEL}:generateContent",headers={"x-goog-api-key":GEMINI_API_KEY},json={"contents":[{"parts":parts}],"generationConfig":{"temperature":0.1,"responseMimeType":"application/json"}},timeout=25);r.raise_for_status();c=r.json().get("candidates",[])
        return c[0]["content"]["parts"][0]["text"] if c else None
...
def save_ai_result(vehicle:Vehicle,data:dict)->tuple[bool,str]:
    kind=str(data.get("type","")).lower().strip();record_date=date.today()
    ...
    try:
        with SessionLocal() as db:
            if kind=="fuel":
                ...
                liters=Decimal(str(data["liters"]));price=Decimal(str(data["price_per_liter"]))
    ...
    except (InvalidOperation,ValueError,TypeError,SQLAlchemyError):return False,"Não foi possível salvar o registro. Revise os dados e tente novamente."
...
def vehicle_form(user:User)->None:
    ...
    with st.form("vehicle_form"):
        ...
        c1,c2,c3=st.columns(3);year=c1.number_input("Ano",min_value=1950,max_value=date.today().year+1,value=existing.year if existing else date.today().year);cf=existing.fuel_type if existing else "Gasolina";fuel=c2.selectbox("Combustível",fuels,index=fuels.index(cf) if cf in fuels else 0);odo=c3.number_input("Quilometragem atual",min_value=0,value=existing.current_odometer if existing else 0)
        ...
        if st.form_submit_button("Salvar veículo",type="primary"):
            ...
            with SessionLocal() as db:
                ...
                v.brand,v.model,v.year,v.fuel_type=brand.strip(),model.strip(),int(year),fuel;v.current_odometer=int(odo);v.version=version.strip() or None;v.license_plate=plate.strip().upper() or None;db.add(v);db.commit()
...
def ai_capture_page(vehicle:Vehicle)->None:
    ...
    image=st.file_uploader("Enviar comprovante ou nota fiscal (opcional)",type=["jpg","jpeg","png","webp"])
    if st.button("Analisar com MOVEXA",type="primary",use_container_width=True):
        ...
        with st.spinner("Analisando..."):raw=ai_request(prompt,image.getvalue() if image else None,image.type if image else "image/jpeg")
        data=parse_ai_json(raw) if raw else None
        if not data:st.error("Não foi possível interpretar a informação. Tente ser mais específico.");return
        st.session_state.ai_capture=data
    data=st.session_state.get("ai_capture")
    if isinstance(data,dict):
        st.markdown('<div class="movexa-section">Confira antes de salvar</div>',unsafe_allow_html=True);st.json(data);c1,c2=st.columns(2)
        if c1.button("Confirmar e salvar",type="primary",use_container_width=True):
            ok,msg=save_ai_result(vehicle,data)
            if ok:st.session_state.pop("ai_capture",None);st.session_state.pop("capture_text",None);st.success(msg);st.rerun()
            else:st.error(msg)
        if c2.button("Descartar",use_container_width=True):st.session_state.pop("ai_capture",None);st.session_state.pop("capture_text",None);st.rerun()
...
def home_page(vehicle:Vehicle)->None:
    ...
    meta=f"{vehicle.year} · {vehicle.current_odometer:,} km · {vehicle.fuel_type}".replace(",",".");st.markdown(f'<div class="movexa-vehicle"><div class="movexa-vehicle-label">Veículo ativo</div><div class="movexa-vehicle-name">{vehicle.brand} {vehicle.model}</div><div class="movexa-vehicle-meta">{meta}</div></div>',unsafe_allow_html=True)
    ...
    if st.button("Processar com MOVEXA",type="primary",use_container_width=True):
        if quick.strip():st.session_state.capture_text=quick;st.session_state.page="Assistente";st.rerun()
    ...
    c=st.columns(4);c[0].metric("Gasto total",money(total));c[1].metric("Combustível",money(sum((Decimal(x.total_cost) for x in mf),Decimal())));c[2].metric("Consumo médio",f"{avg:.2f} km/L" if avg else "—");c[3].metric("Movimentações",len(mf)+len(mm)+len(me))
    st.markdown('<div class="movexa-section">Atividade recente</div>',unsafe_allow_html=True);items=[(x.date,"Abastecimento",f"{x.liters} L · {money(x.total_cost)}") for x in fuels]+[(x.date,"Manutenção",f"{x.description} · {money(x.cost)}") for x in maint]+[(x.date,"Despesa",f"{x.description} · {money(x.amount)}") for x in expenses]
    if items:st.dataframe(pd.DataFrame([{"Data":d.strftime("%d/%m/%Y"),"Tipo":t,"Detalhe":v} for d,t,v in sorted(items,key=lambda x:x[0],reverse=True)[:5]]),use_container_width=True,hide_index=True)
    else:st.info("Sua atividade aparecerá aqui depois do primeiro registro.")

def manual_page(vehicle:Vehicle,page:str)->None:
    if page=="Abastecimentos":
        st.title("Abastecimentos")
        with st.form("fuel_form"):
            c1,c2,c3=st.columns(3);d=c1.date_input("Data",value=date.today(),max_value=date.today());o=c2.number_input("Quilometragem",min_value=0,value=vehicle.current_odometer);l=c3.number_input("Litros",min_value=.001,value=1.0,step=.1);c1,c2,c3=st.columns(3);p=c1.number_input("Preço por litro",min_value=.001,value=1.0,step=.01);ft=c2.selectbox("Combustível",["Gasolina","Etanol","Diesel","Flex","GNV"]);s=c3.text_input("Posto (opcional)")
            if st.form_submit_button("Salvar abastecimento",type="primary"):
                if int(o)<max_odometer(vehicle.id):st.error("A quilometragem não pode ser menor que um registro existente.")
                else:
                    with SessionLocal() as db:db.add(FuelRecord(vehicle_id=vehicle.id,date=d,odometer=int(o),liters=Decimal(str(l)),price_per_liter=Decimal(str(p)),total_cost=Decimal(str(l))*Decimal(str(p)),fuel_type=ft,station=s.strip() or None));db.commit();st.success("Abastecimento salvo.");st.rerun()
    elif page=="Manutenção":
        st.title("Manutenção")
        with st.form("maintenance_form"):
            d=st.date_input("Data",value=date.today(),max_value=date.today());o=st.number_input("Quilometragem",min_value=0,value=vehicle.current_odometer);cat=st.selectbox("Categoria",["Óleo","Pneus","Freios","Revisão","Elétrica","Outros"]);desc=st.text_input("Descrição");work=st.text_input("Oficina (opcional)");cost=st.number_input("Custo",min_value=.01,value=1.0,step=10.)
            if st.form_submit_button("Salvar manutenção",type="primary"):
                if not desc.strip():st.error("Informe a descrição.")
                elif int(o)<max_odometer(vehicle.id):st.error("A quilometragem não pode ser menor que um registro existente.")
                else:
                    with SessionLocal() as db:db.add(MaintenanceRecord(vehicle_id=vehicle.id,date=d,odometer=int(o),category=cat,description=desc.strip(),workshop=work.strip() or None,cost=Decimal(str(cost))));db.commit();st.success("Manutenção salva.");st.rerun()
    elif page=="Despesas":
        st.title("Despesas")
        with st.form("expense_form"):
            d=st.date_input("Data",value=date.today(),max_value=date.today());cat=st.selectbox("Categoria",["Seguro","Imposto","Estacionamento","Pedágio","Lavagem","Outros"]);desc=st.text_input("Descrição");amount=st.number_input("Valor",min_value=.01,value=1.0,step=10.)
            if st.form_submit_button("Salvar despesa",type="primary"):
                if not desc.strip():st.error("Informe a descrição.")
                else:
                    with SessionLocal() as db:db.add(ExpenseRecord(vehicle_id=vehicle.id,date=d,category=cat,description=desc.strip(),amount=Decimal(str(amount))));db.commit();st.success("Despesa salva.");st.rerun()
    else:
        fuels,maint,expenses=load_records(vehicle.id)
        if page=="Histórico":
            items=[(x.date,"Abastecimento",f"{x.liters} L · {money(x.total_cost)}") for x in fuels]+[(x.date,"Manutenção",f"{x.description} · {money(x.cost)}") for x in maint]+[(x.date,"Despesa",f"{x.description} · {money(x.amount)}") for x in expenses];st.title("Histórico");st.dataframe(pd.DataFrame([{"Data":d.strftime("%d/%m/%Y"),"Tipo":t,"Detalhe":v} for d,t,v in sorted(items,key=lambda x:x[0],reverse=True)]),use_container_width=True,hide_index=True) if items else st.info("Ainda não há movimentações.")
        elif page=="Insights":st.title("Insights");st.metric("Gasto total",money(sum((Decimal(x.total_cost) for x in fuels),Decimal())+sum((Decimal(x.cost) for x in maint),Decimal())+sum((Decimal(x.amount) for x in expenses),Decimal())))
        elif page=="Configurações":st.title("Configurações");vehicle_form(current_user())
...
def main()->None:
    ...
    with st.sidebar:
        ...
        page=st.radio("Menu",pages,index=pages.index(st.session_state.page),label_visibility="collapsed");st.session_state.page=page
    if page=="Início":home_page(vehicle)
    elif page=="Assistente":ai_capture_page(vehicle)
    else:manual_page(vehicle,page)
